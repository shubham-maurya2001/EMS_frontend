...
const EditDepartment = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.put(`https://ems-server-drab.vercel.app/api/department/${id}`, department, {
                headers: {
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            setDepLoading(true)
            try {
                const response = await axios.get(`https://ems-server-drab.vercel.app/api/department/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                })
