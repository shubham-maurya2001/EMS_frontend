...
const View = () => {
  ...
  const fetchEmployee = async () => {
    try {
      const response = await axios.get(`https://ems-server-rust.vercel.app/api/employee/${id}`, {
        headers: {
    ...
    employee ? (
      <div className="max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <img className="rounded-full border w-72" src={`https://ems-server-rust.vercel.app/${employee.userId.profileImage}`} alt="" />
          </div>
          <div>
